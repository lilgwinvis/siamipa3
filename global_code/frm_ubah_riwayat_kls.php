					...
					buttons: {
						"Save": function() {
							...
							$.ajax({
				                     ...
				                     url : "ubahkls.php",
				                     cache: false,
				                     data : $('input[type=hidden]').serialize()+"&"+$('input:text').serialize()+"&"+$(":input",oTable.fnGetNodes()).serialize()+"&idx=2&thsmstrnlm="+"<?php echo $thsmstrnlm ?>",
...
<body>
   ...
   <!-- ui-dialog -->
        ...
        ?>  		
		<div id="dialog" title="<?php echo "Ubah Kelas : ".$row['kdkmktrnlm']." - ".$row['nakmktbkmk']." (".$tmp1.")" ?>" >
		<fieldset> 
			  <div id="data">
		<form name="ubahkls" id="ubahkls" method="post" action="ubahkls.php">  
		     <input type="hidden" name="kdmtk" value="<?php echo $row['kdkmktrnlm'] ?>">
			     ...
			     <?php
					...
					?>
						   <tr>	
							...
							<td ><input type="text" name="klsmhs[<?php echo trim($row['nimhsmsmhs']) ?>]" value="<?php echo $row['kelastrnlm'] ?>" id="klsmhs<?php echo $i ?>" ></td>
							<td ><input type="text" name="semmhs[<?php echo trim($row['nimhsmsmhs']) ?>]" value="<?php echo $row['semestrnlm'] ?>" id="semmhs<?php echo $i ?>" ></td>
						   </tr>			
					<?php
					     $i++;
					}				 
